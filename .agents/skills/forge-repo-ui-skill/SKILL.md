...
Create one repository-native UI skill. Do not install a stack of competing design personalities and do not turn this workflow into a universal design encyclopedia.
...
Follow current system, developer, and user instructions first, then the repository's `AGENTS.md` and the instruction sources it routes to, then product and design documents, code, and tests. In this repository `AGENTS.md` limits which `CLAUDE.md` sections bind Codex; the rest of `CLAUDE.md` and `.claude/` is still repository evidence to read during intake, not Codex instructions. This skill supplies a workflow, not project truth.
...
- **Build/synthesize**: research and create the recommended repo-local skill. This does not authorize deleting or replacing an existing skill unless the user explicitly approved replacement or approved a plan naming that deletion.
...
- **Ignore completely**: absent frameworks, wrong artifact types, settled design decisions, unsupported tools, marketing or creative domains the repository does not use, and generic knowledge already encoded locally.

Do not research an excluded category merely because it is popular.

### 3. Research and shortlist

Read [references/research.md](references/research.md) completely. Browse current GitHub sources with the web access exposed in this session; do not rely on a stale remembered catalog. Use seed repositories only as starting points.
...
Score candidates using the rubric in [references/research.md](references/research.md). For every transferable principle, mark:
...
Inventory active design and UI skills and compare trigger descriptions. Recommend one of:

- keep the current skill and improve it;
...
- Never vendor searchable style, font, or palette databases into the generated skill.
...
- Never call self-review independent review. Use a fresh agent only when the session exposes multi-agent tools and the user allows it; spawn it with `fork_turns: "none"` and a self-contained brief. Otherwise label validation direct or static.
